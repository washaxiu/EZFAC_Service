...
def add_checkerInfo_task(cfg):
    ret = False
    if len(cfg) !=0:
        ret = db.insertOrUpdate("CHECKER_INFO",{'fileName': cfg['fileName'], 'checkerEdit': cfg['checkerEdit'], 'isCheck': cfg['isCheck'],'level': cfg['level'],
...
def get_deviceConfig_list_task():
    ...
    if one_group_cfgs:
        for one_group_cfg in one_group_cfgs:
            group_name_infos = {}
            group_name_infos['equipment_id'] = str(one_group_cfg['equipment_id'])
            group_name_infos['current_status'] = str(one_group_cfg['current_status'])
            group_name_infos['product_quantity'] = str(one_group_cfg['product_quantity'])
